...
function Header() {
  ...
  // this function handleSearchSubmit is for to the form to work when we submit 
  // it looks like it does nothing but i think that because
  // we use location.state in our PageSearch, it is storing 
  // the value of the input of our form
  // DO NOT remove or else the search will break!!!
  ...
  return (
    <header className='bg-headerBG'>
      <div className="flex justify-between align-middle items-center text-text h-24">
        <Logo />
        <div className='flex'>
          ...
          />
            ...
            <button onClick={() => setShowNav(!showNav)} className="text-4xl mr-4 md:text-5xl">
              {`${showNav ? "x" : "☰" }`}
            </button>
        </div>
      </div>
